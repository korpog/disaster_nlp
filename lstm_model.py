...
def padding(sentences, length):
    ...
    for i, tweet in enumerate(sentences):
        if len(tweet) != 0:
            features[i, -len(tweet):] = np.array(tweet)[:length]
    return features
...
x_train_padded = padding(x_train, 280)
x_test_padded = padding(x_test, 280)
...
class LSTM_NN(nn.Module):
    def __init__(self, no_layers, vocab_size, hidden_dim, emdedding_dim):
...
for epoch in range(epochs):
    ...
    for tweets, labels in train_loader:
        ...
        model.zero_grad()
        output, hidden = model(tweets, hidden)
    ...
    for tweets, labels in test_loader:
        ...
        model.zero_grad()
        output, hidden_valid = model(tweets, hidden_valid)
...
def predict_text(text):
    ...
    pad =  torch.from_numpy(padding(word_seq,280))
    inputs = pad.to(device)
    batch_size = 1
    h = model.init_hidden(batch_size)
    h = tuple([each.data for each in h])
    output, h = model(inputs, h)
    outcome = 1 if output.item() > 0.5 else 0
    return outcome
...
sample_submission["target"] = test_df["text"].apply(predict_text)

sample_submission.to_csv("submissions/lstm/submission.csv", index=False)
